...
u8 USART_RX_DATA(void)
{
	...
	{
		usart_rx_count = USART_RX_STA;
		USART_RX_STA = 0;
		return usart_rx_count;
	}
	else 
	{
		return USART_RX_NO_OK;
	}
}
...
#define USART2_MAX_RX_SIZE		1024
volatile u32 USART2_RX_STA = 0;
u8 USART2_RX_BUF[USART2_MAX_RX_SIZE+1];
...
void USART2_IRQHandler(void)                	//串口2中断服务程序
	...
	{
		if(USART2_RX_STA < USART2_MAX_RX_SIZE)
		{
...
{
	u32 i;
	for(i = 0;i < USART2_MAX_RX_SIZE;i++)
	{
		USART2_RX_BUF[i] = 0;
	}
	USART2_RX_STA = 0;
...
void USART3_IRQHandler(void)                	//串口2中断服务程序
	...
	{
		if(USART3_RX_STA < USART3_MAX_RX_SIZE)
		{
			//Res =USART_ReceiveData(USART2);	//读取接收到的数据
...
void usart3_rx_buffer_clean(void)
{
	u32 i;
